 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum throws an exception if empty list") {
      assert(sum(List.empty) === 0)
  }
  
  test("sum throws an exception if its argument is 0") {
     intercept[IllegalArgumentException] {
      sum(null)
    }
  }
  ...
  test("max of a few duplicated numbers") {
    assert(max(List(7, 7, 8, 8)) === 8)
  }
  
  test("max throws an exception if empty list") {
    intercept[NoSuchElementException] {
      max(List.empty)
    }
  }
  
  test("max throws an exception if its argument is 0") {
     intercept[NoSuchElementException] {
      max(null)
    }
  }
